angular.module('website').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/views/home/components/sroContact/sroContact.html',
    "<div id=sroContact><div class=entete><h2>- Contact -</h2></div><div class=\"container large-offset-3 large-6\">{{sroContact.contact}}<form ng-submit=sroContact.submit()><div><div class=\"large-6 column\"><label for=contact-name>Nom :</label><input id=contact-name name=name ng-model=sroContact.contact.name required></div><div class=\"large-6 column\"><input id=contact-email type=email name=email ng-model=sroContact.contact.email required></div></div><div><div class=\"large-6 column\"><input id=contact-url type=url name=website ng-model=sroContact.contact.website></div><div class=\"large-6 column\"><input id=contact-subject name=subject ng-model=sroContact.contact.subject required></div></div><div><textarea id=contact-content type=text name=content ng-model=sroContact.contact.content required></textarea></div></form></div><div class=bottom-margin></div></div>"
  );
